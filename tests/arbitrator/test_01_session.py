"""Contract tests for session resource."""
import pytest
...
def test_session_get_returns_all_fields():
    ...
    assert result["playbook"] == "generate"
...
def test_session_get_fails_for_nonexistent_session():
    result = run_arbitrator("session", "get", "--session", "nonexistent-id")
    assert result.returncode != 0


def test_session_create_fails_with_missing_flags():
    result = run_arbitrator("session", "create", "--playbook", "interview")
    assert result.returncode != 0
